      ...
      <div class="panel-body">
        ...
        <table id="viewTable"
          ...
          <tr>
            ...
            <th data-field="sexString" data-filter-control="select">性别</th>
          ...
          <el-form label-position="left" ref="viewModal" :model="viewModal" :rules="rules" autocomplete="off"
            ...
            <div class="modal-body">
              ...
              <el-form-item label="姓名" prop="username">
                <el-input v-model="viewModal.username" clearable/>
              </el-form-item>
              <el-form-item label="性别" prop="sex">
                <el-radio-group v-model="viewModal.sex">
...
<script>
  import $ from 'jquery'
  import {validEmail, validUsername, validPassword, validSex} from '@/utils/validate.js'
  import qs from 'qs'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules: {
          username: [{required: true, min: 3, max: 32, message: "请填入正确的名称", trigger: 'blur'}],
    ...
    methods: {
       ...
       */
      addButton() {
        this.viewModal.modalTitle = this.viewModal.id = this.viewModal.username = this.viewModal.sex = this.viewModal.phone = this.viewModal.mail = ""
      },
      ...
      updateButton: function () {
        ...
        if (rows.length != 1) {
          ...
          this.viewModal.username = row.name;
          this.viewModal.sex = row.sex;
      ...
      removeButton() {
        ...
        if (rows.length <= 0) {
          ...
          this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
            ...
            console.log(ids);
            this.$axios.post("/api/user/removeByIds?ids=" + ids)
              .then(() => {
                this.$message({
                  type: 'success',
                  message: '成功删除数据!'
      ...
      submitForm() {
        this.$refs.viewModal.validate((valid) => {
          if (valid) {
            console.log("验证通过");
            let api = this.viewModal.modalTitle ? '/api/user/updateUser' : '/api/user/setUser'
            let params = {
              id: this.viewModal.id,
              name: this.viewModal.username,
      ...
      resetPassword(e, value, row, index) {
        ...
        this.$axios.post('/api/user/resetPassword' + "?" + params)
          .then((response) => {
            this.$message({
              type: 'success',
              message: '密码重置为123456'
        ...
        $('#viewTable').bootstrapTable({
          ...
          columns: [
            ...
            },
            {
              ...
              align: "center",
              formatter: (value, row, index) => {
                return '<a href="/#/user/UserDetails?id=' + row.id + '">' + row.id + '</a>'
              }
            }],
