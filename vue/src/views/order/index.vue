    ...
    <div>
        ...
        <table id="viewTable"
          ...
          <tr>
            ...
            <th data-field="orderedId" data-sortable="true">订单流水号</th>
            <th data-field="userId" data-sortable="true">用户ID</th>
            <th data-field="orderTypeString" data-sortable="true">订单类型</th>
            <th data-field="status" data-sortable="true">订单状态</th>
          ...
          <el-form label-position="left" ref="viewModal" :model="viewModal" :rules="rules" autocomplete="off"
            ...
            <div class="modal-body">
              ...
              <el-form-item label="订单类型" prop="orderType">
                <el-radio-group v-model="viewModal.orderType">
                  <el-radio label="SHOPCAR">购物车态</el-radio>
                  <el-radio label="WAIT">等待激活</el-radio>
                  <el-radio label="SLOW">暂缓处理</el-radio>
                  <el-radio label="QUICK">加急处理</el-radio>
                  <el-radio label="DEAD">订单死亡</el-radio>
                </el-radio-group>
              </el-form-item>
    ...
    methods: {
      ...
      updateButton: function () {
        ...
        if (rows.length != 1) {
          ...
          this.viewModal.orderType = row.orderType;
      ...
      removeButton() {
        ...
        if (rows.length <= 0) {
          ...
          this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
            ...
            console.log(ids);
            this.$axios.post("/api/order/removeByIds?ids=" + ids)
              .then(() => {
                this.$message({
                  type: 'success',
                  message: '成功删除数据!'
      ...
      submitForm() {
        this.$refs.viewModal.validate((valid) => {
          if (valid) {
            ...
            let api = this.viewModal.modalTitle ? '/api/order/updateOrder' : '/api/order/setOrder'
            let params = {
              id: this.viewModal.id,
              orderType: this.viewModal.orderType,
          ...
          columns: [
            {
              ...
              formatter: (value, row, index) => {
                return '<a href="/#/order/Order?id='+row.id+'">'+row.id+'</a>'
              }
            }],
