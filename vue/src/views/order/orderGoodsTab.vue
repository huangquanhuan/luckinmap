    ...
    <div class="panel panel-default">
      <div class="panel-body">
        <el-tabs type="card" @tab-click="handleClick">
          ...
          <el-tab-pane label="待送达" name="3"></el-tab-pane>
      ...
      <div id="toolbar" class="btn-group">
        ...
        <button v-if="this.status == 2" class="btn btn-info btn-sm rightSize" @click="statusButton">
          <i class="fa fa-cog fa-fw"></i>&nbsp;发货
        </button>
        <button v-if="this.status == 3" class="btn btn-info btn-sm rightSize" @click="statusButton">
          <i class="fa fa-cog fa-fw"></i>&nbsp;送达
        </button>
      </div>
      <table id="viewTable"
        ...
        <tr>
          ...
          <th data-field="suppleId" data-sortable="true" data-visible="false">供应商ID</th>
          <th data-field="userId" data-sortable="true" data-visible="false">用户ID</th>
          <th data-field="userName" data-sortable="true" >购买人名称</th>
          <th data-field="nowPrice" data-sortable="true">购买时价格</th>
          <th data-field="userPhone" data-sortable="true" >购买时电话</th>
        ...
        <div class="modal-content">
          ...
          <el-form label-position="left" ref="viewModal" :model="viewModal" :rules="rules" autocomplete="off"
            ...
            <div class="modal-body">
              ...
              <div class="row">
                <el-form-item label="手机号" prop="userPhone" class="col">
                  <el-input placeholder="请输入手机号" v-model="viewModal.userPhone"/>
                </el-form-item>
                <el-form-item label="用户昵称" prop="userName" class="col">
                  <el-input placeholder="请输入用户昵称" v-model="viewModal.userName"/>
                </el-form-item>
              </div>
              <div class="row">

                <el-form-item label="价格" prop="nowPrice" class="col">
                  <el-input v-model.number="viewModal.nowPrice" disabled></el-input>
                </el-form-item>
                <el-form-item label="地址" prop="userAddress" class="col">
...
<script>
  ...
  import {getGoodsByStatus, updateStatus} from '@/api/order'
  ...
  export default {
    ...
    methods: {
      ...
      statusButton() {
        ...
        if (rows.length <= 0) {
          ...
          console.log("id字符串" + ids);
          return new Promise((resolve, reject) => {
            updateStatus(ids).then(response => {
              const {data} = response
      ...
      removeButton() {
        ...
        if (rows.length <= 0) {
          ...
          this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
            ...
            console.log(ids);
            this.$axios.post("/api/order/removeGoodsByIds?ids=" + ids)
              .then(() => {
                this.$message({
                  type: 'success',
                  message: '成功删除数据!'
    ...
    watch: {
      status: function () {
        ...
        const promise = this.getGoods(params);
        promise.then((data) => {
