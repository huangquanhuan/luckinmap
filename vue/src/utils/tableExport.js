...
(function ($) {
  $.fn.tableExport = function (options) {
    ...
    let pageFormats = { // Size in pt of letious paper formats. Adopted from jsPDF.
    ...
    if ( defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt' ) {
      ...
      let csvString = function (cell, rowIndex, colIndex) {
        let result = '';
      ...
      $(el).filter(function () {
        ...
        let rowLength = CollectXmlssData (collectHeadRows ($table), 'th,td', 0);
      ...
      let wbout = XLSX.write(workbook, {type: 'binary', bookType: defaults.mso.fileFormat, bookSST: false});
            ...
            function (cell, row, col) {
              if ( cell !== null ) {
                ...
                if ( defaults.mso.styles.length ) {
                  let cellstyles = document.defaultView.getComputedStyle(cell, null);
                  let rowstyles = document.defaultView.getComputedStyle($row[0], null);
                ...
                let tdcolspan = getColspan (cell);
                if ( tdcolspan > 0 )
                  trData += ' colspan="' + tdcolspan + '"';

                let tdrowspan = getRowspan (cell);
                if ( tdrowspan > 0 )
      ...
      if ( defaults.pdfmake.enabled === true ) {
         ...
         * @return {number}
         */
        let CollectPdfmakeData = function ($rows, colselector, length) {
          let rlength = 0;
        ...
        let colcount = CollectPdfmakeData($hrows, 'th,td', $hrows.length);
        ...
        let doc = new jsPDF(defaults.jspdf.orientation, defaults.jspdf.unit, defaults.jspdf.format);
        ...
        if ( typeof defaults.jspdf.format === 'string' && defaults.jspdf.format.toLowerCase() === 'bestfit' ) {
          ...
          $(el).each(function () {
            if ( isVisible($(this)) ) {
              let w = getPropertyUnitValue($(this).get(0), 'width', 'pt');
        ...
        loadImages(teOptions, function () {
          $(el).filter(function () {
            ...
            let atOptions                        = $.extend(true, {}, defaults.jspdf.autotable);
            ...
            if ( typeof atOptions.beforePageContent !== 'function' ) {
              atOptions.beforePageContent = function (data) {
                if ( data.pageCount === 1 ) {
                  let all = data.table.rows.concat(data.table.headerRow);
                  $.each(all, function () {
                    let row = this;
                    if ( row.height > 0 ) {
            ...
            if ( typeof atOptions.createdHeaderCell !== 'function' ) {
              ...
              atOptions.createdHeaderCell = function (cell, data) {
                ...
                if ( typeof teOptions.columns [data.column.dataKey] !== 'undefined' ) {
                  let col = teOptions.columns [data.column.dataKey];
            ...
            if ( typeof atOptions.createdCell !== 'function' ) {
              ...
              atOptions.createdCell = function (cell, data) {
                let tecell = teOptions.teCells [data.row.index + ":" + data.column.dataKey];
            ...
            if ( typeof atOptions.drawHeaderCell !== 'function' ) {
              atOptions.drawHeaderCell = function (cell, data) {
                let colopt = teOptions.columns [data.column.dataKey];
            ...
            if ( typeof atOptions.drawCell !== 'function' ) {
              atOptions.drawCell = function (cell, data) {
                let tecell = teOptions.teCells [data.row.index + ":" + data.column.dataKey];
                let draw2canvas = (typeof tecell !== 'undefined' && tecell.isCanvas);
                ...
                if ( draw2canvas !== true ) {
                  ...
                  let container = tecell.elements[0];
                  let imgId  = $(container).attr("data-tableexport-canvas");
    ...
    function findTableElements ($parent, selector) {
      let parentSelector = $parent[0].tagName;
      let parentLevel = $parent.parents(parentSelector).length;
    ...
    function isVisible ($element) {
      ...
      let isCell = isRow === false && typeof $element[0].cellIndex !== 'undefined';
      let isElementVisible = (isCell || isRow) ? isTableElementVisible($element) : $element.is(':visible');
    ...
    function ForEachVisibleCell (tableRow, selector, rowIndex, rowCount, cellcallback) {
      if ( typeof (cellcallback) === 'function' ) {
        ...
        if (ignoreRow === false &&
          ...
          let $cells = findTableElements($(tableRow), selector);
          let cellCount = 0;
    ...
    function jsPdfDrawImage (cell, container, imgId, teOptions) {
      if ( typeof teOptions.images !== 'undefined' ) {
        let image = teOptions.images[imgId];

        if ( typeof image !== 'undefined' ) {
          let r          = container.getBoundingClientRect();
          let arCell     = cell.width / cell.height;
          let arImg      = r.width / r.height;
    ...
    function prepareAutoTableText (cell, data, cellopt) {
      ...
      if ( cs >= 0 ) {
        ...
        let i         = data.table.columns.indexOf(data.column);

        for ( let c = 1; c < cs; c++ ) {
          let column = data.table.columns[i + c];
        ...
        if ( cell.styles.valign === 'middle' || cell.styles.valign === 'bottom' ) {
          let splittedText = typeof cell.text === 'string' ? cell.text.split(/\r\n|\r|\n/g) : cell.text;
          let lineCount    = splittedText.length || 1;
          if ( lineCount > 2 )
      ...
      if ( typeof cell !== 'undefined' && cell !== null ) {
        ...
        if ( cell.hasAttribute("data-tableexport-canvas") ) {
          ...
          elements.each(function () {
            if ($(this).is("img")) {
              let imgId = strHashCode(this.src);
              teOptions.images[imgId] = {
                url: this.src,
                src: this.src
              };
    ...
    function drawAutotableElements (cell, elements, teOptions) {
      elements.each(function () {
        if ( $(this).is("div") ) {
          ...
          let ux = this.offsetLeft * teOptions.wScaleFactor;
          let uy = this.offsetTop * teOptions.hScaleFactor;
          let uw = r.width * teOptions.wScaleFactor;
          let uh = r.height * teOptions.hScaleFactor;
      ...
      if ( typeof teOptions.onAutotableText === 'function' ) {
        ...
        if ( texttags.length ) {
          ...
          while ( tag ) {
            ...
            let leadingspace = (txt.length && txt[0] === " ") ? " " : "";
            let trailingspace = (txt.length > 1 && txt[txt.length - 1] === " ") ? " " : "";
    ...
    function parseString (cell, rowIndex, colIndex) {
      ...
      if ( cell !== null ) {
        ...
        if ( defaults.htmlContent === true ) {
          ...
          let cellFormat = $(cell).attr("data-tableexport-cellformat");
          ...
          if ( cellFormat !== '' ) {
            ...
            if ( defaults.type === 'json' ||
              ...
              if ( number !== false ) {
                let frac = ("" + number.substr(number < 0 ? 1 : 0)).split('.');
                if ( frac.length === 1 )
                  frac[1] = "";
    ...
    function xlsxTableToSheet(table) {
      ...
      for(; _R < rows.length && R < sheetRows; ++_R) {
        ...
        for(_C = C = 0; _C < elts.length; ++_C) {
          ...
          let v = parseString(elt,_R,_C + CSOffset);
          let o = {t:'s', v:v};
          let _t = elt.getAttribute("t") || "";
          if(v != null) {
    ...
    function xlsxToDate(s) {
      let o = new Date(s), n = new Date(NaN);
      let y = o.getFullYear(), m = o.getMonth(), d = o.getDate();
      if(isNaN(d)) return n;
